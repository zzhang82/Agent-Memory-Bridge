...
Current package/source version is `0.31.0`. The current source/package line exposes 17 public MCP tools and includes the read-only Knowledge Explorer CLI. The pinned `v0.27.0` release is a historical published baseline with the same public surface:
...
evidence. The current 0.31.0 source/package line uses schema v12 with the implemented Knowledge Explorer over existing governed-v2
