...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot，以及 `297 passed`。
...
更复杂的能力留在 surface 后面：reflex promotion、consolidation、startup/task-time assembly、procedure governance、telemetry summaries、signal contention checks，以及 learning-candidate review queues。当前没有单独的 `task_packet`、`startup_packet` 或 `learning_candidate` MCP tools。
...
`0.15.0` 是 reviewed memory revision 和 forgetting-gate release，建立在 governed learning-candidate lane 之上，同时保持 public tool surface 稳定。
