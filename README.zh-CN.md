...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot，以及 `323 passed`。
...
`0.19.0` 是 proof-breadth release：它把 v0.19 adoption plan 落成一个可执行的 12-case fixture pack，覆盖 retrieval、Task Brief behavior 和 first-run client guidance；不会写 client config 或 durable memory records，同时保持 public MCP surface 稳定。
