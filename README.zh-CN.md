...
`0.22.3` 在不增加 MCP tools 的前提下收紧现有 correctness contract。Signal polling 现在按升序完整读取后续插入；active claim 只能由 owner ack；promotion 会保留 relation、validity 和 lineage metadata；operational JSONL 写入失败也不会推翻已提交的业务操作。Public MCP surface 仍为 10 个 tools。
...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot、visual inventory checks，以及 `405 passed`。
...
`0.22.3` 修复了经过运行验证的 Signal polling、claimed-work ack、structured record parsing、promotion preservation 和 post-commit operational output 问题。一次本地 acceptance run 用 `limit=100` 完整读取 10,000 个 Signals，保持精确插入顺序，missing 和 duplicate 都为零。测试套件还包含 8 个独立进程争抢同一 ID、最终只有 1 个 winner 的用例。这些是本地 SQLite/WAL 范围内的保证，不代表 exactly-once delivery、distributed queue、authenticated ownership 或 namespace access control。
...
| v0.22.3 correctness | 10,000-Signal polling acceptance：精确插入顺序，`missing = 0`, `unexpected = 0`, `unique = 10000`；suite 覆盖 active-claim owner ack 和 promotion preservation regression |
