...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot，以及 `302 passed`。
...
更复杂的能力留在 surface 后面：reflex promotion、consolidation、startup/task-time assembly、procedure governance、telemetry summaries、signal contention checks，以及 learning-candidate review queues。当前没有单独的 `task_packet`、`startup_packet`、`learning_candidate` 或 `review_queue` MCP tools。
...
它会显示 staged candidates、review receipts、tombstones、stale records 和 quarantined claims，并给出 proposal-only next steps。
...
`0.16.0` 是 reviewed memory operations release：它在 staged candidates、review receipts、tombstones、stale records 和 quarantined claims 之上增加只读 operator review queue，同时保持 public tool surface 稳定。
