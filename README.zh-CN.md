...
`0.23.0` 在不增加 MCP tools 的前提下加固本地 retrieval maintenance 和后台 service。Semantic recall、scheduled embedding maintenance 和 embedding rebuild 现在会在 SQLite write transaction 之外调用 provider；hybrid recall 遇到明确的 provider failure 时会退回 lexical results；service cycle 中抛出的 exceptions 会按 lane 隔离并使用 bounded backoff；lane state 使用 atomic write；schema upgrade 具备有序、事务化的版本 spine。中文/Han text 也可以进入 hash-semantic retrieval。Public MCP surface 仍为 10 个 tools。
...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot、visual inventory checks，以及 `445 passed`。

## 工作方式

<p align="center">
  <img src="examples/diagrams/amb-overview.svg" alt="Agent Memory Bridge 架构：通用 MCP-compatible coding agents 通过分组的 10 个工具访问 SQLite/WAL authority、derived indexes、governed change，以及不会自动 durable writeback 的 context 和 reports；proof gates 独立于 runtime。" width="760">
</p>

AMB 保持 runtime path 很小：MCP-compatible coding agents 调用 `10` 个
...
正常 always-on service 使用时，Codex-log watcher capture、reflex promotion 和 strong consolidation 默认关闭。每个 cycle 中，启用的 lane 都有独立 exception boundary：单个 lane failure 会返回 failure count 和 bounded retry delay，但不会停止其他 lanes。Lanes 仍按顺序执行，因此慢调用仍会延迟后续 lanes。Watcher、reflex、consolidation、governance 和 embedding scheduler state 共用同一套 tolerant atomic JSON contract。
...
`0.23.0` 加固了经过运行验证的本地 reliability paths。Semantic recall、scheduled maintenance 和 embedding rebuild 会在 SQLite write transaction 之外批量调用 provider，并在写入 vector 前重新检查 content hash。Hybrid mode 在配置的 embedding provider 失败时返回带明确 degraded metadata 的 lexical results；显式 semantic mode 仍会清楚失败。后台 lanes 使用独立 exception boundary 和 bounded backoff；state files 使用 tolerant atomic replacement；schema initialization 现在记录有序、事务化的 migrations。中文/Han text 已覆盖 hash-semantic retrieval；当 FTS 没有直接命中时，中文 lexical retrieval 仍使用现有 LIKE fallback。
...
| v0.23.0 reliability | semantic recall、scheduler 和 rebuild 的 provider work 位于 write transaction 之外；typed hybrid degradation；isolated service lanes；atomic state replacement；ordered schema version `1`，并覆盖 rollback 与 multi-process convergence regression |
