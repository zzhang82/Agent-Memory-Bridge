...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot，以及 `336 passed`。
...
`0.20.0` 是 clean-room adoption proof release：它用隔离的本地临时 store 跑真实 stdio MCP entrypoint，完成 tokened `store -> recall` round trip，渲染 first-run guidance 和 Task Brief；不会写 client config，也不要求 AMH。
