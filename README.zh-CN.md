...
`0.22.0` 增加 Cross-Client Activation Receipt：client A 存入一条 reviewed project memory，client B recall 后确认一条 read signal，本地 CLI 生成 sanitized receipt，证明同一个 correlation 下参与了两个不同的声明式 `source_client` label。这只是 declared provenance，不是 authenticated identity、vendor certification 或 external adoption evidence。
...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot，以及 `388 passed`。
...
`0.22.0` 保留固定的 v0.21 governed-change proof，并增加一个本地 activation receipt，用于证明一次 shared-memory loop 中有两个声明式 client labels 参与。它仍然是边界明确的本地 memory system；本版本不声称通用 machine unlearning、graph-memory traversal、隐私合规、vendor certification、authenticated client identity、external adoption 或 automatic policy enforcement。Tombstone 只审计被删除的 record ID；它不会阻止调用方以后把相同内容显式写成一个新的 ID。
