...
`0.23.1` 完整处理上一版本审计中发现的可靠性缺口。Classifier suggestions 不能生成 policy tags；后台 cursor 能处理相同 timestamp 和 database restore；本地 service 具备 singleton ownership、heartbeat、slow-lane timing 和明确的 one-shot exit code。存储层新增 typed metadata、indexed lineage、projection repair、一致性 backup/restore、WAL maintenance、严格本地 profile、bounded command provider，以及显式 `annotate` / `revise` 操作。Public MCP surface 现在是 12 个 tools。
...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot、visual inventory checks，以及 `546 passed`。
...
`0.23.1` 增加有 regression 覆盖的 authority、storage、retrieval 与本地 daemon 加固。Classifier output 不能生成 governance tags，confidence 必须存在、为有限值且位于 `[0, 1]`；command provider 默认 `shell=False` 并限制 I/O。Reflex、consolidation、embedding scheduler 和 Signal polling 使用 database-generation-aware cursor 或 state。Typed projections 与 indexed lineage 支持短事务 governed delete、derived state repair、显式 annotation/revision 和一致性 backup/restore。跨平台 OS file lock 防止普通重复 service；heartbeat、slow-lane timing、one-shot failure、database health 和 Signal repair 都对 operator 可见。
...
| 当前 reliability patch | policy-tag 隔离；epoch-aware cursor；singleton lock 与 heartbeat；typed metadata/lineage；backup/restore/checkpoint；bounded command provider；exact full-store semantic scoring；strict local profile |
