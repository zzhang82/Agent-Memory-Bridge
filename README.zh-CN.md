...
- Proof discipline：release contract、public-surface check、onboarding check、benchmark snapshot，以及 `312 passed`。
...
`0.18.0` 是 Task Brief context-assembly release：它新增一个 read-only CLI report，把已有 task-memory assembly、review queue items 和 active signals 编译成 `Used`、`Ignored` 和 `Needs Review` 区块，同时保持 public MCP surface 稳定。
