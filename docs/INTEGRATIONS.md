...
The current source/package line is `0.31.1` with exactly 17 public MCP tools. Knowledge Explorer is a CLI-only, read-only derived projection over existing project knowledge; use a source checkout to evaluate an exact checkout and consult GitHub Releases for live publication availability.
