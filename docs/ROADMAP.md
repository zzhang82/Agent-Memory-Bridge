...
| Knowledge Explorer and observability | Explore a local-first, initially read-only inspection surface for memory, procedure, state, run, outcome, lineage, and context-selection visibility. | Views, clustering, and reflection remain derived and non-authoritative; no dashboard becomes a write authority by implication. |
