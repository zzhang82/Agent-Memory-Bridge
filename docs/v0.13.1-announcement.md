...
- Static-schema MCP clients can send signal-only fields on `kind="memory"` paths
  without breaking memory creation or later recall, browse, and export.
  The normalization happens only at the MCP server boundary; the lower-level
  memory store still keeps durable memory and coordination signals separate.
