...
from agent_mem_bridge.cole_migration import compare_cole_migration_with_mode
...
def main() -> None:
    ...
    preflight = compare_cole_migration_with_mode(store, source_root, mode="live")
    if not args.force:
        if (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Move archive-first Cole markdown out of the live source tree.")
    parser.add_argument(
        "source_root",
        nargs="?",
        type=Path,
    ...
    parser.add_argument(
        "--cutover-root",
        type=Path,
        help="Optional destination for retired live-source files. Defaults to <Cole>/archive/<date>-live-cutover.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
