...
def test_path_resolvers_read_from_config_file(tmp_path: Path, monkeypatch) -> None:
    ...
    assert resolve_cole_source_root() == tmp_path / "remote-cole"
...
def test_env_overrides_config_values(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("COLE_SOURCE_ROOT", str(tmp_path / "env-cole"))

    assert resolve_cole_source_root() == tmp_path / "env-cole"
