...
def test_import_profile_memory_imports_only_supported_markdown(tmp_path: Path) -> None:
    ...
    (profile_root / "memory" / "executors" / "spawn_worker.py").write_text("print('skip')\n", encoding="utf-8")

    store = MemoryStore(db_path=tmp_path / "bridge.db", log_dir=tmp_path / "logs")
    result = import_profile_memory(store, profile_root)

    assert result["document_count"] == 4
    assert result["comparison"]["missing_count"] == 0

    core_hits = store.recall(namespace="profile-core", query="Calm and direct", limit=10)
    workflow_hits = store.recall(namespace="profile-workflows", query="Own the contract", limit=10)
    skill_hits = store.recall(namespace="profile-skills", query="Obsidian Markdown", limit=10)
...
def test_compare_profile_migration_detects_missing_docs(tmp_path: Path) -> None:
    profile_root = tmp_path / "Profile"
    (profile_root / "memory" / "core").mkdir(parents=True)
    persona_path = profile_root / "memory" / "core" / "persona.md"
    persona_path.write_text("# Persona\n\nCalm and direct.\n", encoding="utf-8")

    store = MemoryStore(db_path=tmp_path / "bridge.db", log_dir=tmp_path / "logs")
    documents = build_profile_documents(profile_root)
    assert len(documents) == 1

    comparison_before = compare_profile_migration(store, profile_root)
    assert comparison_before["missing_count"] == 1

    import_profile_memory(store, profile_root)
    comparison_after = compare_profile_migration(store, profile_root)
...
def test_compare_profile_migration_detects_content_mismatch(tmp_path: Path) -> None:
    ...
    comparison = compare_profile_migration(store, profile_root)
...
def test_live_compare_uses_manifest_instead_of_full_source_tree(tmp_path: Path) -> None:
    ...
    (profile_root / "memory" / "core" / "persona.md").write_text("# Persona\n\nCalm and direct.\n", encoding="utf-8")
    workflow_path = profile_root / "memory" / "workflows" / "subagent-patterns.md"
    ...
    live_comparison = compare_profile_migration_with_mode(store, profile_root, mode="live")
    full_comparison = compare_profile_migration_with_mode(store, profile_root, mode="full")

    assert live_comparison["content_mismatch_count"] == 0
    assert live_comparison["extra_count"] == 0
    assert live_comparison["out_of_scope_count"] == 2
    assert live_comparison["out_of_scope_paths"] == [
        "HOW-TO-USE-PROFILE.md",
        "memory/workflows/subagent-patterns.md",
    ]
...
def test_snapshot_audit_compares_against_snapshot_manifest(tmp_path: Path) -> None:
    ...
    snapshot_root = build_default_snapshot_root(profile_root)
...
def test_prune_stale_profile_imports_removes_extra_paths(tmp_path: Path) -> None:
    ...
    before = compare_profile_migration(store, profile_root)
    assert before["extra_paths"] == ["skills/obsidian-markdown/SKILL.md"]

    pruned = prune_stale_profile_imports(store, profile_root)
    after = compare_profile_migration(store, profile_root)
...
def test_compare_profile_migration_accepts_legacy_import_source_app(tmp_path: Path) -> None:
    profile_root = tmp_path / "Profile"
    (profile_root / "memory" / "core").mkdir(parents=True)
    persona_path = profile_root / "memory" / "core" / "persona.md"
    persona_path.write_text("# Persona\n\nCalm and direct.\n", encoding="utf-8")
    ...
    store = MemoryStore(db_path=tmp_path / "bridge.db", log_dir=tmp_path / "logs")
    relative_path = Path("memory/core/persona.md")
    store.store(
        namespace="profile-core",
        kind="memory",
        title="Persona",
        content=render_document_content(persona_path, relative_path, "profile-core"),
        tags=build_document_tags(relative_path, "profile-core"),
        actor=IMPORT_ACTOR,
        source_app=LEGACY_IMPORT_SOURCE_APP,
    )
